...
class TestPokemon {
    ...
    async getPokemons(type, value) {
        ...
        if (pokemon["status"] == this.checkObject.status) {
            ...
            logger.info("Test Pass -> pokemon with type " + type + " and value " + value + " is exist")
        }
        else {
            logger.error("Test Fail -> status " + pokemon["status"])
    ...
    async deletePokemon(type, value) {
        ...
            logger.info(`Status Code: ${this.managerApi.deletePokemon.status}`)
        ...
        if(!(ob['body'] instanceof(Array)))
        {
            logger.info("Test Pass -> the pokemon with " + type + " and value " + value + " deleted")
             ...
             logger.error("Test Fail -> the pokemon with type" + type + "and value" + value + " not deleted")
...
//testPokemon.deletePokemon("id", "5")
